...
class middleware {
      ...
      if (!contentType()) {
        return res
          ...
          .json({
            status: 500,
            success: false,
            message: "JSON tipida so`rov yuborilishi shart",
          });
      }
      next();
    } catch {
  ...
  async checkParamsId(req, res, next) {
    try {
      if (typeof req.params.id !== "string" || req.params.id.length !== 24) {
        res
          .status(500)
          .json({ status: 500, success: false, message: "Id xato" });
        return;
      }
      next();
    } catch {
  ...
  async checkReferenceId(req, res, next) {
    try {
      ...
      if (func(req.body) < 1) {
        res
          ...
          .json({
            ...
            success: false,
            message:
              "Faoliyat qo`shish uchun hatyabi bitta reference id ber, qayoga qo`shey u becharani :(",
          });
        return;
      }
      if (func(req.body) > 1) {
        res
          ...
          .json({
            ...
            success: false,
            message:
              "Faoliyat qo`shish uchun faqat bitta reference id ber, qaysi birini qo`shey bulani :(",
          });
        return;
      }
      next();
      ...
      res
        .status(500)
        .json({ status: 500, success: false, message: "invalid request" });
      return;
    }
  }
      ...
      if (func(req.body) > 1) {
        res
          ...
          .json({
            ...
            success: false,
            message:
              "Faoliyat qo`shish uchun reference id bittadan ko`p bo`lishi mumkin emas!",
          });
        return;
      }
