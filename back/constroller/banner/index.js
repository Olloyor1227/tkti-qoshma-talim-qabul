...
class BannerController {
  async Add(req, res) {
    try {
      if (req.file && req.file.mimetype.split("/")[0] !== "image") {
        ...
        res
          .status(403)
          .json({ status: 403, message: "faqat rasm yuklay olasiz" });
        return;
      }
      ...
      if (!req.file) {
        res
          ...
          .json({
            status: 403,
            message: `Banner uchun rasm qo'yilishi majburiy`,
            success: false,
          });
        return;
      }
    ...
    try {
      ...
      if (!oldbanner) {
        ...
        res
          ...
          .json({
            status: 404,
            message: `Tahrirlamoqchi bo'lgan banner topilmadi :(`,
            success: false,
          });
        return;
      }
      ...
      const updated = await banner.findByIdAndUpdate(req.params.id, value, {
        new: true,
      });
  ...
  async GetById(req, res) {
    try {
      ...
      if (!Banners) {
        res
          .status(404)
          .json({ status: 404, message: "topilmadi", success: false });
        return;
      }
      res
        .status(200)
  ...
  async Delete(req, res) {
    try {
      ...
      res
        .status(200)
        .json({ status: 200, message: `yaxshi o'chirildi`, data: Banners });
    } catch (e) {
      console.log(e);
      res
        .status(500)
