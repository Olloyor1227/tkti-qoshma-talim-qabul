...
exports.updateNews = async (req, res) => {
  try {
    ...
    const news = await News.findOneAndUpdate({ _id: req.params.id }, value, {
      new: true,
    });
    if (!news) {
      res
        ...
        .json({
          status: 403,
          message: "Yangilanishi kerak bo'lgan yangilik topilmadi",
        });
      return;
    }
    res.status(200).json({
  ...
  try {
    ...
    if (!news) {
      res
        ...
        .json({
          status: 403,
          message: "O'chishi kerak bo'lgan yangilik topilmadi",
        });
      return;
    }
    res.status(200).json({
